 ...
 */
function gt_pure_customize_register_options( $wp_customize ) {

	// Add Theme Options Panel.
	$wp_customize->add_panel( 'gt_pure_options_panel', array(
		'priority'       => 180,
		'capability'     => 'edit_theme_options',
		'theme_supports' => '',
		'title'          => esc_html__( 'Theme Options', 'gt-pure' ),
	) );

	// Change default background section.
	$wp_customize->get_control( 'background_color' )->section = 'background_image';
	$wp_customize->get_section( 'background_image' )->title   = esc_html__( 'Background', 'gt-pure' );
}
add_action( 'customize_register', 'gt_pure_customize_register_options' );


/**
 * Embed JS file to make Theme Customizer preview reload changes asynchronously.
 */
function gt_pure_customize_preview_js() {
	wp_enqueue_script( 'gt-pure-customize-preview', get_template_directory_uri() . '/assets/js/customize-preview.js', array( 'customize-preview' ), '20200303', true );
	...
	if ( class_exists( 'GermanThemes_Local_Fonts' ) ) {
		...
		wp_localize_script( 'gt-pure-customize-preview', 'gtPureFontSettings', $font_settings );
	}
}
add_action( 'customize_preview_init', 'gt_pure_customize_preview_js' );


/**
 * Embed JS for Customizer Controls.
 */
function gt_pure_customizer_controls_js() {
	wp_enqueue_script( 'gt-pure-customizer-controls', get_template_directory_uri() . '/assets/js/customizer-controls.js', array(), '20200303', true );
}
add_action( 'customize_controls_enqueue_scripts', 'gt_pure_customizer_controls_js' );


/**
 * Embed CSS styles Customizer Controls.
 */
function gt_pure_customizer_controls_css() {
	wp_enqueue_style( 'gt-pure-customizer-controls', get_template_directory_uri() . '/assets/css/customizer-controls.css', array(), '20200303' );
}
add_action( 'customize_controls_print_styles', 'gt_pure_customizer_controls_css' );
