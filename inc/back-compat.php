 ...
 *
 * Prevents GT Drive from running on WordPress versions prior to 5.2,
 ...
 * GT Drive on WordPress versions prior to 5.2.
 *
 * @global string $wp_version WordPress version.
 */
function gt_drive_upgrade_notice() {
	$message = sprintf( esc_html__( '%1$s requires at least WordPress version %2$s. You are running version %3$s. Please upgrade and try again.', 'gt-drive' ), 'GT Drive', '5.2', $GLOBALS['wp_version'] );
...
function gt_drive_customize() {
	wp_die( sprintf( esc_html__( '%1$s requires at least WordPress version %2$s. You are running version %3$s. Please upgrade and try again.', 'gt-drive' ), 'GT Drive', '5.2', $GLOBALS['wp_version'] ), '', array(
		'back_link' => true,
	) );
}
...
function gt_drive_preview() {
	if ( isset( $_GET['preview'] ) ) {
		wp_die( sprintf( esc_html__( '%1$s requires at least WordPress version %2$s. You are running version %3$s. Please upgrade and try again.', 'gt-drive' ), 'GT Drive', '5.2', $GLOBALS['wp_version'] ) );
	}
}
